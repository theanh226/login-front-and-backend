...
router.get("/handle/:currentHandle", (req, res) => {
  const errors = {};

  Profile.findOne({ handle: req.params.currentHandle })
    .populate("currentUser", ["name", "avatar"])
    .then(profile => {
      if (!profile) {
        errors.noprofile = "There is no profile for this user";
...
router.get("/user/:user_id", (req, res) => {
  ...
  Profile.findOne({ currentUser: req.params.user_id })
    .populate("currentUser", ["name", "avatar"])
    .then(profile => {
      if (!profile) {
        errors.noprofile = "There is no profile for this user";
...
router.delete(
  ...
  passport.authenticate("jwt", { session: false }),
  (req, res) => {
    Profile.findOneAndRemove({ currentUser: req.user.id }).then(() => {
